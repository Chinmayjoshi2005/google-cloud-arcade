...
```bash
gcloud compute instances create multi-nic-vm --zone=[Enter Zone] --machine-type=e2-medium --network-interface=network=my-vpc1,subnet=subnet-a --network-interface=network=my-vpc2,subnet=subnet-b
```
